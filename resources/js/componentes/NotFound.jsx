...
const NotFound = () => {
  ...
  return (
    <div ref={ref} className='back-gradien xs:pt-16 lg:pt-16 text-center pb-12 min-h-screen	flex flex-col items-center justify-center'>
      <h1 className={`${animationUp} xl-text-450 xs:text-4xl md:text-6xl 2xl:text-9xl text-white font-bold font-time`}>404</h1>
      <h2 className={`text-white ${animationUp} md:text-6xl xs:text-xl`}>Página no encontrada</h2>
      <h3 className={`${animationUp} text-white md:text-3xl xs:text-xl`}>¡OPPS! PÁGINA NO ENCONTRADA</h3>
...
if(document.getElementById('no-encontrado')){
    createRoot(document.getElementById('no-encontrado')).render(<NotFound/>)
}
