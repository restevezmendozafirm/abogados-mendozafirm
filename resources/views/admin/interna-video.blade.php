...
@section('content')
    <div class="bg-dashboard">
        <div class="back-home bg-center py-20 md:pt-40 md:pb-20 md:px-8 items-center relative justify-center flex flex-col" style="background-image: url(/images/banner-dashboard.webp) !important; background-position: bottom">
            <div class="max-w-screen-2xl mx-auto w-full">
                <div class="max-w-3xl px-4 md:px-0">
                    <p class="2xl:text-7xl xl:text-6xl xs:text-4xl z-50 text-white font-bold md:mb-10 mb-4 md:mt-0 mt-20">MasterClass by MGM</p>
        ...
        <div class="mt-12 pb-16" id="class">
            <div class="max-w-screen-xl xl:max-w-screen-lg block mx-auto px-4 2xl:px-0">
                <div class="relative">
                    {!! $video !!}
                    <div class="overlay flex justify-center items-center cursor-pointer" onclick="togglePlay()">
                        <div class="play-icon cursor-pointer md:w-60 md:h-60 w-20 h-20"></div>
                    </div>
                </div>
                <script>
                    const video = document.querySelector('.relative video');
                    const overlay = document.querySelector('.overlay');

                    function togglePlay() {
                        if (video.paused) {
                            video.play(); // Reproducir el video
                            overlay.style.display = 'none'; // Ocultar el filtro oscuro
                        } else {
                            video.pause(); // Pausar el video
                            overlay.style.display = 'flex'; // Mostrar el filtro oscuro
                        }
                    }
                </script>
                <p class="md:text-3xl text-base text-center font-bold tgreen-youtube md:mb-8 md:mt-12 mt-4">{{$capacitacion->nombre}}</p>
                <p class="text-center md:text-xl xs:text-sm md:mb-8 xs:mb-4 font-bold lg:font-normal">{{$capacitacion->description}}</p>
                <div class="text-center">
                    <a href="{{$capacitacion->link_archivo}}" target="_blank" class="bg-grey py-2 px-8 rounded-full text-black text-base md:text-2xl font-bold">MORE INFORMATION</a>
                </div>
            </div>
            <div class="max-w-screen-2xl lg:grid block xl:grid-cols-3 lg:grid-cols-3 gap-8 xl:gap-12 mx-auto px-16 lg:px-4 2xl:px-0 mt-16">
